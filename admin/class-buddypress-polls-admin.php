			...
			switch ( $column_name ) {
				...
				case 'pollstatus':
					...
					if ( $never_expire == 1 ) {
						...
						if ( new DateTime( $start_date ) > new DateTime() ) {
							echo '<span class="dashicons dashicons-calendar"></span> ' . __( 'Yet to Start', 'buddypress-polls' );                      } else {
				...
				if ( $total_percent > 0 ) {
					...
					foreach ( $poll_result['weighted_index'] as $index => $vote_count ) {
						...
						if ( isset( $poll_ans_image[ $index ] ) && ! empty( $poll_ans_image[ $index ] ) && empty( $thumbnail_poll_ans_image[ $index ] ) ) {
							$output_result .= '<div class="wbpoll-question-choices-item-content-container"><div class="wbpoll-question-choices-item-content"><div class="poll-image"><span class="poll-image-view" data-id="' . $index . '">view image</span><img src="' . $poll_ans_image[ $index ] . '"></div></div></div>';
