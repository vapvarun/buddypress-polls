...
class WBPollHelper
{
    ...
    public static function create_wbpoll_post_type()
    {
        ...
        $args = array(
            ...
            'supports'        => apply_filters('wbpoll_post_type_supports', array(
                'title',
                'editor',
                'author',
                'thumbnail'
    ...
    public static function check_array_element_value_type($array)
    {
        ...
        if (!empty($array)) {
            foreach ($array as $val) {
                $ret[] = WBPollHelper::check_value_type($val);
            }
        }
    ...
    {
        $states = array(
            '0' => esc_html__('Unapproved', 'wbpoll'),
            '1' => esc_html__('Approved', 'wbpoll'),
            '2' => esc_html__('Spam', 'wbpoll'),
            '3' => esc_html__('Unverified', 'wbpoll')
        );
    ...
    {
        $states = array(
            'unapprove'  => esc_html__('Unapproved', 'wbpoll'),
            'approve'    => esc_html__('Approved', 'wbpoll'),
            'spam'       => esc_html__('Spam', 'wbpoll'),
            'unverified' => esc_html__('Unverified', 'wbpoll')
        );
    ...
    public static function user_roles($plain = true, $include_guest = false, $ignore = array())
    {
        ...
        if ($plain) {
            ...
            if ($include_guest) {
                $userRoles['guest'] = esc_html__("Guest", 'wbpoll');
            }
        } else {
            //optgroup
            $userRoles_r = array();
       ...
       // $setting_api  = $settings = new WBPoll_Settings();
        ...
        if ($poll_status !== 'publish') {
            ...
            if (in_array('administrator', $this_user_role) || in_array('editor', $this_user_role)) {
                ...
                return esc_html__('Sorry, poll is not published yet or poll doesn\'t exists.', 'wbpoll');
            }
        ...
        $show_description               = intval(get_post_meta($post_id, '_wbpoll_content',
            true)); //show poll description or not
        $poll_never_expire              = intval(get_post_meta($post_id, '_wbpoll_never_expire',
            true)); //poll never epire
        $poll_show_result_before_expire = intval(get_post_meta($post_id, '_wbpoll_show_result_before_expire',
            true)); //poll never epire
        $poll_result_chart_type         = get_post_meta($post_id, '_wbpoll_result_chart_type', true); //chart type
        $poll_is_voted                  = WBPollHelper::is_poll_voted($post_id);
        ...
        if (new DateTime($poll_start_date) <= new DateTime()) {
            ...
            if ($reference != 'content_hook') {
                ...
                if (intval($description) == 1) {
                    ...
                    $poll_output .= '<div class="wbpoll-description">'.apply_filters('wbpoll_description',
            ...
            if ($is_poll_expired) { // if poll has expired
                ...
                if ($answers_by_user !== null) {
                    ...
                    if ($answers_by_user_html != "") {
                        $poll_output .= '<p class="wbpoll-voted-info wbpoll-voted-info-'.$post_id.'">'.sprintf(__('The Poll is out of date. You have already voted for <strong>"%s"</strong>',
                                'wbpoll'), $answers_by_user_html).' </p>';
                    } else {
                        $poll_output .= '<p class="wbpoll-voted-info wbpoll-voted-info-'.$post_id.'"> '.sprintf(__('The Poll is out of date. You have already voted for <strong>"%s"</strong>',
                                'wbpoll'), $answers_by_user_html).' </p>';
                    ...
                    $poll_output .= '<p class="wbpoll-voted-info wbpoll-voted-info-'.$post_id.'"> '.__('The Poll is out of date. You have not voted.',
                            'wbpoll').'</p>';
                }
                ...
                if ((sizeof($allowed_user_group)) < 1) {
                    ...
                    if ($poll_show_result_before_expire == 1) {
                        if ($poll_is_voted) {
                            $poll_output .= WBPollHelper::show_single_poll_result($post_id, $reference,
                                $result_chart_type);
                        }
                    ...
                    if ($poll_is_voted_by_user) {
                        ...
                        if ($answers_by_user !== null) {
                            ...
                            if ($answers_by_user_html != "") {
                                $poll_output .= '<p class="wbpoll-voted-info wbpoll-voted-info-'.$post_id.'">'.sprintf(__('You have already voted for <strong>"%s"</strong>',
                                        'wbpoll'), $answers_by_user_html).' </p>';
                            } else {
                                $poll_output .= '<p class="wbpoll-voted-info wbpoll-voted-info-'.$post_id.'">'.esc_html__('You have already voted ',
                                        'wbpoll').' </p>';
                        ...
                        $poll_form_html .= '<ul class="'.apply_filters('wbpoll_form_answer_list_style_class',
                        ...
                        foreach ($poll_answers as $index => $answer) {
                            ...
                            $extra_list_style = '';
                            $extra_list_attr  = '';
                            $poll_form_html   .= '<li class="'.apply_filters('wbpoll_form_answer_listitem_style_class',
                                    $poll_answer_listitem_class, $post_id, $index, $answer,
                                    $poll_answers_extra_single).'" style="'.apply_filters('wbpoll_form_answer_listitem_style',
                                    $extra_list_style, $post_id, $index, $answer,
                                    $poll_answers_extra_single).'" '.apply_filters('wbpoll_form_answer_listitem_attr',
            ...
            $poll_output = esc_html__('Poll Status: Yet to start', 'wbpoll');
        }

        $poll_output .= '</div>'; //end of wbpoll_wrapper

        return $poll_output;
    }//end method wbpoll_single_display
    ...
    public static function show_single_poll_result($poll_id, $reference, $result_chart_type = 'text')
    {
        ...
        $poll_never_expire              = intval(get_post_meta($poll_id, '_wbpoll_never_expire',
            true)); //poll never epire
        $poll_show_result_before_expire = intval(get_post_meta($poll_id, '_wbpoll_show_result_before_expire',
            true)); //poll never epire


        $poll_result_chart_type = get_post_meta($poll_id, '_wbpoll_result_chart_type', true); //chart type

        $result_chart_type = WBPollHelper::chart_type_fallback($result_chart_type);
        ...
        $poll_display_methods = WBPollHelper::wbpoll_display_options();
    ...
    public static function get_meta_fields()
    {
        ...
        $roles           = WBPollHelper::user_roles(false, true);
        $global_settings = get_option('wbpoll_global_settings');


        $default_user_roles = isset($global_settings['user_roles']) ? $global_settings['user_roles'] : WBPollHelper::user_roles(true,
            true);
        ...
        $poll_display_methods = WBPollHelper::wbpoll_display_options();
        $poll_display_methods = WBPollHelper::wbpoll_display_options_linear($poll_display_methods);
        ...
        $post_meta_fields = array(
            ...
            '_wbpoll_start_date'   => array(
                'label'   => esc_html__('Start Date', 'wbpoll'),
            ...
            '_wbpoll_end_date'     => array(
                'label'   => esc_html__('End Date', 'wbpoll'),
            ...
            '_wbpoll_user_roles'   => array(
                'label'    => esc_html__('Who Can Vote', 'wbpoll'),
                'desc'     => esc_html__('Which user role will have vote capability', 'wbpoll'),
            ...
            '_wbpoll_content'      => array(
                'label'   => esc_html__('Show Poll Description in shortcode', 'wbpoll'),
                'desc'    => esc_html__('Select if you want to show content.', 'wbpoll'),
                'id'      => '_wbpoll_content',
                'type'    => 'radio',
                'default' => $default_content,
                'options' => array(
                    '1' => esc_html__('Yes', 'wbpoll'),
                    '0' => esc_html__('No', 'wbpoll')
                )

            ),
            '_wbpoll_never_expire' => array(
                'label'   => esc_html__('Never Expire', 'wbpoll'),
            ...
            '_wbpoll_show_result_before_expire' => array(
                'label'   => esc_html__('Show result before expires', 'wbpoll'),
            ...
            '_wbpoll_multivote'                 => array(
                'label'   => esc_html__('Enable Multi Choice', 'wbpoll'),
                'desc'    => esc_html__('Can user vote multiple option', 'wbpoll'),
            ...
            '_wbpoll_vote_per_session' => array(
                'label'   => esc_html__('Votes per session', 'wbpoll'),
                'desc'    => esc_html__('Votes per session', 'wbpoll'),
                'id'      => '_wbpoll_vote_per_session',
                'type'    => 'number',
                'default' => 1,
            ),
            '_wbpoll_timeout' => array(
                'label'   => esc_html__('Timeout', 'wbpoll'),
                'desc'    => esc_html__('How many times can the user vote using the same session', 'wbpoll'),
                'id'      => '_wbpoll_timeout',
                'type'    => 'radio',
                'default' => 0,
                'options' => array(
                    '1' => esc_html__('30 Minutes', 'wbpoll'),
                    '2' => esc_html__('1 Hour', 'wbpoll'),
                    '3' => esc_html__('6 Hour', 'wbpoll'),
                    '4' => esc_html__('1 Day', 'wbpoll'),
                    '5' => esc_html__('1 Week', 'wbpoll'),
                    '6' => esc_html__('1 Month', 'wbpoll'),
                    '7' => esc_html__('6 Month', 'wbpoll'),
                    '8' => esc_html__('1 Year', 'wbpoll')
                )
            ),
        );
    ...
    }//end method wbpoll_answer_field_template
	...
	public static function guest_login_forms() {
		...
		$forms['wordpress'] = esc_html__( 'WordPress Core Login Form', 'wbpoll' );

		return apply_filters( 'wbpoll_guest_login_forms', $forms );
	}//end guest_login_forms

}//end class WBPollHelper